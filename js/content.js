			...
			switch (request.action) {
				...
				case "query": {
					...
					if (input && input.value !== request.query) {
						...
						// Enhanced: More realistic typing with variable delays
						...
						// Enhanced: Trigger additional events for better compatibility
				...
				case "perform": {
					...
					// Enhanced: Try multiple methods to submit the search
					...
					if (form) {
						...
						// Enhanced: Alternative - simulate Enter key press
				...
				case "validateResults": {
					// Enhanced: Validate that search results are actually present
					const hasResults = document.querySelector("#b_results, .b_algo, .b_searchResult") !== null;
					const hasSearchBox = document.querySelector("#sb_form_q") !== null;
					const isSearchPage = window.location.href.includes('bing.com');
					
					sendResponse({ 
						success: hasResults && hasSearchBox && isSearchPage,
						details: {
							hasResults,
							hasSearchBox,
							isSearchPage,
